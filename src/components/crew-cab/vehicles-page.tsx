...
type Vehicle = { id: string; make: string; model: string; year: number | null; registration: string | null; color: string | null; capacity: number; mileage: number; status: string; insurance_expiry: string | null; notes: string | null };
const EMPTY = { make: "", model: "", year: "", registration: "", color: "", capacity: "4", mileage: "0", status: "available", insurance_expiry: "", notes: "" };

function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = { available: "bg-emerald-500/15 text-emerald-400", in_use: "bg-blue-500/15 text-blue-400", maintenance: "bg-amber-500/15 text-amber-400" };
  const label: Record<string, string> = { available: "Available", in_use: "In Use", maintenance: "Maintenance" };
  return <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${map[status] ?? "bg-muted text-muted-foreground"}`}>{label[status] ?? status}</span>;
}
...
export function VehiclesPage() {
  ...
  async function load() {
    ...
    if (error) toast.error(error.message); else setVehicles(data as Vehicle[]);
    setLoading(false);
  }
  ...
  async function handleDelete(v: Vehicle) {
    ...
    const { error } = await (supabase as any).from("crew_vehicles").delete().eq("id", v.id);
    if (error) toast.error(error.message); else { toast.success("Vehicle removed"); await load(); }
  }
    ...
    <div className="flex h-full flex-col">
      <header className="flex items-center justify-between border-b border-border bg-card/40 px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground ml-1">({vehicles.length})</span>
        </div>
        <Button onClick={openNew} size="sm" className="gap-1.5"><Plus className="h-4 w-4" /> Add Vehicle</Button>
      </header>
      <div className="flex-1 overflow-auto p-6">
        {loading ? (
          <div className="flex h-40 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div>
        ) : vehicles.length === 0 ? (
          <div className="flex h-40 flex-col items-center justify-center gap-2 text-muted-foreground"><Car className="h-10 w-10 opacity-30" /><p className="text-sm">No vehicles yet. Add your first vehicle.</p></div>
        ) : (
          <div className="rounded-lg border border-border overflow-x-auto">
            <table className="w-full text-sm min-w-[900px]">
              <thead className="bg-muted/40 border-b border-border">
                <tr>{["Make","Model","Year","Registration","Color","Capacity","Mileage","Insurance Expiry","Status",""].map(h => <th key={h} className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground whitespace-nowrap">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-border">
                {vehicles.map(v => (
                  <tr key={v.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground font-mono text-xs">{v.registration ?? "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{v.color ?? "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{v.capacity}</td>
                    <td className="px-4 py-3 text-muted-foreground">{v.mileage?.toLocaleString()} km</td>
                    <td className="px-4 py-3 text-muted-foreground">{v.insurance_expiry ?? "—"}</td>
                    <td className="px-4 py-3"><StatusBadge status={v.status} /></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <Button variant="ghost" size="sm" onClick={() => openEdit(v)} className="h-7 w-7 p-0"><Pencil className="h-3.5 w-3.5" /></Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(v)} className="h-7 w-7 p-0 text-destructive/70 hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5"><Label>Status</Label>
              <Select value={form.status} onValueChange={v => setForm(f => ({ ...f, status: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="available">Available</SelectItem><SelectItem value="in_use">In Use</SelectItem><SelectItem value="maintenance">Maintenance</SelectItem></SelectContent></Select>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-border">
              ...
              <Button onClick={handleSave} disabled={busy} className="gap-1.5">{busy && <Loader2 className="h-4 w-4 animate-spin" />}{editing ? "Save Changes" : "Add Vehicle"}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
